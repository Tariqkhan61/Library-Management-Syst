...
st.set_page_config(
    ...
    page_icon="📚",  # Added a book emoji as the page icon
    layout="wide",
...
lottie_book = load_lottieurl("https://assets9.lottiefiles.com/packages/lf20_1a8dx7zj.json")  # Book animation
if lottie_book:
    with st.sidebar:
...
if st.session_state.current_view == "add":
    ...
    with st.form(key='add_book_form'):
        ...
        with col2:
            genre = st.selectbox("Genre", [
                "Fiction", "Non-Fiction", "Science", "Technology", "Religious", "Poetry", "Self-Help", "History"
            ])
        ...
        if submit_button and title and author:
            with st.spinner("Adding book..."):  # Loading animation
            ...
            with cols[i % 2]:
                ...
                with col1:
                    if st.button(f"Remove", key=f"remove_{i}", use_container_width=True):
                        with st.spinner("Removing book..."):  # Loading animation
                            time.sleep(1)  # Simulate loading
                            if remove_book(i):
                                st.rerun()
                with col2:
    ...
    if st.button("Search", use_container_width=True):
        if search_term:
            with st.spinner("Searching..."):  # Loading animation
                time.sleep(1)  # Simulate loading
                search_books(search_term, search_by)
    
    # Check if search_results exists and display results
    if hasattr(st.session_state, 'search_results'):  # Corrected attribute name
        if st.session_state.search_results:  # Corrected attribute name
...
st.markdown("### Copyright @ 2025 📚📕 Muhammad Tariq Mahboob Ⓜ️ Personal Library Manager", unsafe_allow_html=True)